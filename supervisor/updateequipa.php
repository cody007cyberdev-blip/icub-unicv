...
// Consulta para obter os dados da equipe e do projeto associado ao supervisor atual
$queryEquipa = "
    SELECT e.id, e.nomeequipa, e.id_projetos, p.nome_projeto
    FROM tbl_equipa e
    INNER JOIN tbl_projeto p ON e.id_projetos = p.id
    INNER JOIN tbl_supervisor s ON p.id_supervisor = s.id
    WHERE e.id = :id_equipa AND s.id = :id_supervisor
";
$stmtEquipa = $conexion->prepare($queryEquipa);
$stmtEquipa->bindParam(':id_equipa', $idEquipa);
$stmtEquipa->bindParam(':id_supervisor', $_SESSION['usuario']['id']); // Assuming $_SESSION['usuario']['id'] contains supervisor's ID
$stmtEquipa->execute();
...
if (!$equipa) {
    ...
    flashMessage("Equipa não encontrada ou você não tem permissão para acessá-la", 'error');
    redirect('equipa.php');
    exit;
}

// Consulta para obter todos os projetos disponíveis para o supervisor atual que não têm equipe associada
$queryProjetos = "
    SELECT p.id, p.nome_projeto
    FROM tbl_projeto p
    LEFT JOIN tbl_equipa e ON p.id = e.id_projetos
    WHERE p.id_supervisor = :id_supervisor AND (e.id IS NULL OR e.id = :id_equipa)
";
$stmtProjetos = $conexion->prepare($queryProjetos);
$stmtProjetos->bindParam(':id_supervisor', $_SESSION['usuario']['id']); // Assuming $_SESSION['usuario']['id'] contains supervisor's ID
$stmtProjetos->bindParam(':id_equipa', $idEquipa);
$stmtProjetos->execute();
$projetos = $stmtProjetos->fetchAll(PDO::FETCH_ASSOC);

// Consulta para obter todos os candidatos disponíveis que não estão em nenhuma equipe ou estão na equipe atual
$queryCandidatos = "
...
<div class="container col-10 p-4">
    ...
    <?php elseif (FlashMessage('success')) : ?>

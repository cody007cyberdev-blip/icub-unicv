...
// verificarAutenticacao('candidato');
if (isset($_GET['projetoID'])) {
    // dado o parametro projetoID
    $projetoID = $_GET['projetoID'];
    // pegar todos os dados do projeto
    $projeto = (new Model('tbl_projeto'))->get($projetoID);
    // pegar o supervisor deste projeto
    $supervisor = (new Model('tbl_supervisor'))->get($projeto['id_supervisor']);
    // encontrar todos os cadidatos relacionados a este projeto
    $equipa = (new Model('tbl_candidato'))->findAll('id_projeto', $projetoID);
} else {
    // die('ID de Projeto Invalido');
}
?>

<h1>

    Aqui é onde fica todo o workflow do projeto para o utilizador aprovado e funcionamento com o projeto
